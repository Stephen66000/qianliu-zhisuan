...
### PC-20260729-09：仟流 IDE 移出一期范围

| 项目 | 内容 |
| --- | --- |
| 日期 | 2026-07-29 |
| 提出／批准 | 佳哥 |
| 变化事实 | 仟流 IDE 为第一方自研客户端，当前开发进度缓慢。PRD §12.1 原列其为一期接入方式（"员工登录后自动取得 Gateway、模型和受控凭证"），但 §12.2 已声明"不以仟流 IDE 支持 Windows 作为一期完成条件""仟流 IDE 首期只在 2 台 Mac 验证"。为避免自研进度阻塞一期交付，将仟流 IDE 移出一期必需客户端范围。 |
| 决策 | 一期客户端 DoD 用 WorkBuddy、Claude Code、ZCode（及 Kimi/VS Code 等 OpenAI/Anthropic 兼容工具）闭合；仟流 IDE 不作为 M6/M7 完成条件，做好后以增强形式加回。Owner 澄清：仟流 IDE 与上述工具一样填 Key 接入（不做员工账密登录），故移出不影响 Gateway 接入代码（Bearer Key 鉴权 M2 已就绪）。预埋的 `employee_login` 表保留不启用。 |
| 影响范围 | PRD §12.1（IDE 标注移出一期）、§12.2（Mac IDE 验证项移出一期矩阵）、M6 DoD 第 1 条"每工具≥2 真实员工"不再含 IDE、W21（Mac IDE 现场回归移出一期） |
| 不变项 | Gateway 北向协议合同、Bearer Key 鉴权、四客户端统一接入模型、产品目标与 scope 主体、M6 代码（协议矩阵/5 秒门禁已完成） |
| Evidence | 本变更登记；M6 双审任务书已记录口径 |

### PC-20260729-10：M6 真实员工实测门禁延后至部署后与 M7 一起闭合

| 项目 | 内容 |
| --- | --- |
| 日期 | 2026-07-29 |
| 提出／批准 | 佳哥 |
| 变化事实 | M6 DoD 第 1 条"每个工具至少 2 名真实员工"本质依赖部署：没有可访问的服务器，员工无法真实使用客户端连接。原计划排序 M6 在 M7（部署）之前，使"真实员工实测"在部署前闭合成为不可能——这是计划排序的不现实之处。 |
| 决策 | M6 代码部分（协议矩阵冻结、停用/重置即时生效、Bearer Key 接入）已完成并双审 READY_WITH_ACTIONS；M6 真实员工实测门禁与 Win11 实机回归（W22）延后至 M7 部署后，随内部 Pilot（W26）一起闭合。M6 不阻塞 M7 部署代码（W24 依赖 M1-M6 代码，非最终签字）。 |
| 影响范围 | M6 最终签字时点（延后）、M6/W22 门禁闭合路径（绑定 M7 部署 + W26 Pilot）、详细计划里程碑排序说明 |
| 不变项 | M6 DoD 三条本身（实测仍需做，只是时点延后）、M7 DoD、外部门禁如实声明原则（禁止伪装） |
| Evidence | 本变更登记；stage-state next_action 已反映 |
