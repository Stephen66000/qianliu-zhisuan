...
### PC-20260731-11：新增“运行保障”一级模块与企微成员定向通知

| 项目 | 内容 |
| --- | --- |
| 日期 | 2026-07-31 |
| 提出／批准方向 | 佳哥 |
| 变化事实 | Kimi／智谱真实接入暴露出熔断语义混杂、连续失败误隔离、规则只能新增不能编辑、错误路由恢复死锁、告警依赖管理员打开页面才评估，以及受影响人员无法主动获知等问题。现有 PRD 将外部通知列为非目标，现有一级导航只有“异常告警”，不足以承载完整运行保障闭环。 |
| 产品决定 | 新增一级菜单“运行保障”，保留运行态势、熔断规则、熔断事件、异常中心、通知配置五个功能区；规则必须支持添加、编辑、版本、启停、历史和回滚；客户端至少显示中文原因与恢复信息。 |
| 熔断边界 | 硬熔断只由上游明确原始信号或管理员计划熔断规则触发；5xx、超时、网络错误、连续失败和失败率只预警／降级，不得自行触发硬熔断。 |
| 人员与通知 | 先建立自然人“人员”对象，再映射企微 userid；人主体关联人员，项目主体必须绑定项目负责人。第一版通过企业微信自建应用按 userid 直接发送给本人，不使用通知群或群机器人；只发送触发和恢复消息，同事件同人员去重。 |
| 试验范围 | 当前仅服务一家公司约 10 名内部用户；规则、资源、人员和通知配置共享同一工作空间，不建设多租户、多企业配置或企业间数据隔离能力。 |
| 影响范围 | PRD scope／non-scope、TRD 资源状态机与异常检测、一级导航、主体模型、Local First 本地基线收敛、数据库迁移、Gateway 新增错误合同、Provider Adapter、厂商 Caller、持续 Worker／周期任务、部署环境开关、Control API、Web、部署、测试和 Evidence。 |
| 不变项 | 正文零留存、Secret 不回显、逐 Attempt 与账本合同、流式提交后不切换、外部源码边界、真实 Provider 分别上线签字。 |
| 风险 | 主本地项目目录与 Mac Mini 已部署迁移／源码不完整一致、Adapter 分类被生产路径旁路、厂商 Caller／业务码解析缺失、资源迁移出现双权威、原始信号误映射、企微应用 Secret／token 泄露、成员不在应用可见范围、项目负责人缺失、当前 Worker 无持续调度能力、平台运行模式误配置影响全部试验用户。 |
| 决策门禁 | 本条记录确认产品方向；业务编码仍须先完成并由 Owner 确认独立开发计划，随后同步 PRD／TRD、领取下一迁移编号并授权 RA-W01。 |
| 开发计划 | [仟流智算“运行保障”一级模块开发计划 v1.0](./仟流智算-运行保障一级模块开发计划-v1.0.md) |

### PC-20260801-12：审核整改授权与历史 RA-W01 Evidence Gap 分离

| 项目 | 内容 |
| --- | --- |
| 日期 | 2026-08-01 |
| 当前指令 | Owner 在当前 Codex 任务中明确要求“请依次修复”。 |
| 当前授权范围 | 仅处理本轮审核 Findings：Worker 生命周期、依赖安全、质量门禁、状态事实源、代码质量和同候选 Evidence；不包含 commit、push、PR、merge 或再次生产发布。 |
| 历史事实 | 仓库中仍未找到 PC-20260731-11 要求的独立 RA-W01 授权载体。不得用当前指令倒签或改写历史授权事实。 |
| 恢复执行点 | `AUDIT-REMEDIATION-20260801`，先修 P1，再处理 P2，最后锁定新候选并执行 HIGH 风险门禁。 |
| 影响范围 | 当前候选状态回到 `doing`；历史 `dd80e975` 首次生产发布事实继续保留，但其结论不自动覆盖新候选。 |
| 复查触发 | 候选内容变化、质量门禁失败、出现 P0/P1、要求生产发布或扩大真实流量验证。 |
| 整改结果 | 本轮 P1/P2 已完成；候选对象锁摘要为 `c5344ad925ba0fdc4c58f1769bff4765ab759d2213fa85a00e7bb84148e18ee3`，功能、安全、恢复、代码质量 Evidence 位于 `V3/Evidence/Final-Audit/20260801-remediation/`。业务验证仍独立保持 `doing/证据不足`，未伪装为生产验证通过。 |
