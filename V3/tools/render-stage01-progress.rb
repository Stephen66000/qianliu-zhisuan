...
html_path = File.join(root, "V3", "仟流智算-Stage01方案准备进度图-v0.3.html")
...
snapshot = {
  ...
  "productName" => state.fetch("product_name"),
  "projectVersion" => state.fetch("product_version"),
  "productOwner" => state.fetch("product_owner"),
  "techOwner" => state.fetch("tech_owner"),
  "updatedAt" => state.fetch("updated_at"),
  "stage" => state.fetch("stage"),
...
pattern = %r{(<script type="application/json" id="stage-state-snapshot">\n).*?(\n</script>)}m
abort "stage-state-snapshot marker missing" unless html.match?(pattern)
