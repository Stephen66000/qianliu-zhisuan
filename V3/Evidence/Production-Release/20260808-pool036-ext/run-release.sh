#!/usr/bin/env bash
# 部署首页统计排除归档/停用员工 + 接入配置折叠未就绪型号（commit 93bdfa7）。
#
# 本次改动（POOL-008/009 验收遗留清理的延伸）：
#   - 首页活跃人数/在用人数/员工榜排除归档/停用员工（status=ACTIVE AND archived_at IS NULL）
#   - 单人接入配置页未就绪型号折叠为详情，不再平铺灰显
#
# 无数据库迁移（纯查询过滤 + 前端展示）。数据库应保持 0042_alias_ql_format 不变。
# 线上当前版本：POOL-038 bf6fcb3（本次回滚目标）。
set -Eeuo pipefail
...
candidate_commit="93bdfa7e0103b5138ae99971df3fef33098747b7"
release="/Users/stephen/releases/qianliu-zhisuan-pool036ext-93bdfa7-$(date '+%Y%m%d')"
previous="$(cat /Users/stephen/qianliu-current-release.txt)"
stamp="$(date '+%Y%m%d-%H%M%S')"
backup_dir="/Users/stephen/backups/qianliu-zhisuan"
backup="${backup_dir}/pre-pool036ext-${stamp}.dump"
log_file="${release}/deploy-pool036ext-${stamp}.log"
...
rollback() {
  ...
  if test "$paused" = 1; then
    ...
    for service in control-api gateway worker web; do
      docker image tag "qianliu-rollback-93bdfa7-${service}" "qianliu-zhisuan-${service}" || true
    done
...
log "数据库基线=${latest_before}（确在 0042，本次不迁移）"
...
# ========== 3. 冻结回滚镜像（回到当前线上 bf6fcb3）==========
log "step 3: freezing rollback images (回到当前线上版本 bf6fcb3)"
for service in control-api gateway worker web; do
  old_id="$(docker inspect --format '{{.Image}}' "qianliu-zhisuan-${service}-1")"
  test -n "$old_id"
  docker image tag "$old_id" "qianliu-rollback-93bdfa7-${service}"
done
log "已冻结回滚镜像"
...
log "迁移号确认为 ${final_latest}（本次无迁移，应保持 0042）"
...
log "首页统计排除归档/停用员工 + 接入配置折叠未就绪型号 已就绪"
