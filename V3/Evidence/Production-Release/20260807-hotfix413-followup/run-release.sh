#!/usr/bin/env bash
# 部署 hotfix-413-followup：审核报告 P2 修复 F-2/F-3/G-1/G-2/B-1（commit ca77666）。
#
# 本次改动（hotfix413 初版的 follow-up，均为 P2 改进，非紧急）：
#   F-2  keepTokens 倒序累加 break 改为累加后判定，消除 off-by-one
#   F-3  tool 配对保护补反向场景（assistant 在但 tool 响应被截 → 丢弃 assistant）
#   G-1/G-2 截断测试断言强化（锁定具体保留条数/被丢内容/配对完整性）
#   B-1  setErrorHandler 兜底补 req.log 日志（复刻 Fastify 默认行为）
#   附带 applyHistoryTruncation 的 log 参数收窄为 TruncationLogger
#
# 无数据库迁移（纯应用层改动）。数据库应保持 0041_provider_quota_window 不变。
# 线上当前版本：hotfix413 初版 21e5a20（本次回滚目标）。
#
# 审核状态：follow-up P2，已两轮审核 + 61 单测全过。
set -Eeuo pipefail
...
candidate_commit="ca77666659193075b11a1ae0ed4ed28c17c62d38"
release="/Users/stephen/releases/qianliu-zhisuan-hotfix413-followup-ca77666-$(date '+%Y%m%d')"
previous="$(cat /Users/stephen/qianliu-current-release.txt)"
stamp="$(date '+%Y%m%d-%H%M%S')"
backup_dir="/Users/stephen/backups/qianliu-zhisuan"
backup="${backup_dir}/pre-hotfix413-followup-${stamp}.dump"
log_file="${release}/deploy-hotfix413-followup-${stamp}.log"
...
rollback() {
  ...
  if test "$paused" = 1; then
    ...
    for service in control-api gateway worker web; do
      docker image tag "qianliu-rollback-ca77666-${service}" "qianliu-zhisuan-${service}" || true
    done
...
# ========== 3. 冻结回滚镜像（回到当前线上 21e5a20）==========
log "step 3: freezing rollback images (回到当前线上版本 21e5a20)"
for service in control-api gateway worker web; do
  old_id="$(docker inspect --format '{{.Image}}' "qianliu-zhisuan-${service}-1")"
  test -n "$old_id"
  docker image tag "$old_id" "qianliu-rollback-ca77666-${service}"
done
log "已冻结回滚镜像"
...
log "hotfix-413-followup 已就绪：F-2/F-3/G-1/B-1 修复已上线；截断仍默认关闭"
