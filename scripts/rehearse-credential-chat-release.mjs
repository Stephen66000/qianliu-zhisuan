...
const source = "28f6a603c29986b56985f9e305a82cb6c51a9253";
const sourceTree = "2928ab83891701833d19c68502443eb506291a3c";
const scriptPath = "deploy/scripts/release-credential-chat-20260910-mac-mini.sh";
...
const tree =
  original.match(/^candidate_tree=([0-9a-f]{40})$/m)?.[1] ??
...
const suiteRoot = realpathSync(
  mkdtempSync(path.join(tmpdir(), "qianliu-release-rehearsal-")),
);
...
if (cmd === 'git') {
  ...
  if (args.includes('rev-parse')) {
    ...
    if (args.includes('--name-status')) out(scenario === 'migration-scope-mismatch' ? 'M\\told-migration.js' : 'A\\tpackages/database/migrations/0073_credential_chat_probe.js');
...
if (args[0] === 'inspect') {
  ...
  if (args.includes('exec')) {
    ...
    s.names = [...s.sourceNames, '0073_credential_chat_probe'];
...
for (const scenario of [
  ...
  const script = original
    .replace("server_root=/Users/stephen", "server_root=" + root)
  ...
  if (scenario === "success") {
    assert.equal(state.names.at(-1), "0073_credential_chat_probe");
    assert.equal(
      readFileSync(root + "/qianliu-current-release.txt", "utf8").trim(),
      state.current,
    );
