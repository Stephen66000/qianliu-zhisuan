...
const pg = await startPostgresContainer("session_revoke_e2e"), db = createKysely(pg.connectionString), app = buildControlApi(db);
...
try {
  ...
  expect(unauthorized).toBeGreaterThanOrEqual(1); expect(unauthorized).toBeLessThanOrEqual(3);
  const dir = root + "/V4/Evidence/SYSTEM-SETTINGS-V2-20260909/REPAIR";
  await mkdir(dir, { recursive: true });
  await page.screenshot({ path: dir + "/F01-login-after-revoke.png" });
  const observed401 = unauthorized;
  await signIn();
  const result = { status: "PASS", revoked: revoke.statusCode, observed401, stableLoginPage: true, relogin: true, database: "isolated" };
