...
describe.sequential("标准版首页聚合（getStandardHomeSummary）", () => {
  ...
  it("production integration preserves historical plan purchases in the prior window", async () => {
    const r = await createResource({providerCode:"history-home",providerName:"History",name:"History plan",mode:"CODING_PLAN"});
    const start=new Date("2025-07-31T16:00:00Z"), end=new Date("2025-08-10T06:00:00Z");
    await db.insertInto("resource_purchase_record").values({enterprise_id:enterpriseId,provider_resource_id:r.resourceId,purchase_type:"PACKAGE_PURCHASE",amount:"199",currency:"CNY",purchased_at:new Date("2025-08-05T00:00:00Z"),source:"ADMIN",created_by:adminId}).execute();
    const window=await loadWindowOperatingFinance(db,enterpriseId,start,end);
    expect(window.totalSpends).toEqual([{currency:"CNY",amount:"199.00000000"}]);
    expect(window.incompleteReason).toBeNull();
  });

  it("production integration excludes empty failed audit records from cost gaps", async () => {
    const r=await createResource({providerCode:"audit-home",providerName:"Audit",name:"Audit API",mode:"API"});
    const at=new Date("2025-06-05T00:00:00Z");
    await addLine({principalId:employeeA,keyId:keyA,resourceId:r.resourceId,at,input:0n,output:0n,requestStatus:"FAILED",apiCostStatus:"UNKNOWN_COST"});
    const window=await loadWindowOperatingFinance(db,enterpriseId,new Date("2025-05-31T16:00:00Z"),new Date("2025-06-10T06:00:00Z"));
    expect(window.incompleteReason).toBeNull();
  });
