...
it("migration up/down/up is reversible before instructions or system facts exist", async () => {
  const migrator=createMigrator(db); const down=await migrator.migrateTo("0065_principal_accounting_assignment");expect(down.error).toBeUndefined();
  expect(down.results?.at(-1)?.migrationName).toBe("0066_subscription_auto_renewal");
...
it("cancellation keeps existing periods and money, blocks later renewal and is not undone by replay", async () => {
  ...
  const down=await createMigrator(db).migrateTo("0065_principal_accounting_assignment");
  expect(String(down.error)).toContain("0066 rollback blocked");
  await migrateToLatest(db);
});
