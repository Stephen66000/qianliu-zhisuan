/**
 * 项目归集批次执行（候选 C3 合同 §5）：认领（SKIP LOCKED + 租约）、计算、
 * 原子发布（先关旧 current）、幂等命中 no-op、失败退避与 Worker 入口。
 * 从 project-allocation-run-repository 拆出（80 终审 P1-3 体量门禁）。
 */
...
import {
  ...
  type EmployeeAllocationContext, type EmployeeRuleSegment,
} from "@qianliu/domain";
...
export async function executeAllocationRun(
  ...
  try {
    ...
    await db.transaction().execute(async (tx) => {
      await publishOrNoopRun(tx, {
        run, inputDigest, ruleDigest, membershipDigest, manualDigest,
        result, lineById, monthRange, financeEnabled, employees, projects,
        resultHash, started, lines,
      });
