...
import { BillingRulePersistence } from "./billing-rule-persistence.js";
...
export class GatewayLedgerRepository {
  ...
  // Billing APIs remain available on the existing ledger repository.
  async listActiveBillingRules(
    enterpriseId: string, at: Date,
  ): ReturnType<BillingRulePersistence["listActiveBillingRules"]> {
    return new BillingRulePersistence(this.db).listActiveBillingRules(enterpriseId, at);
  }

  async listAllBillingRules(
    enterpriseId: string,
    archived: "exclude" | "only" | "all" = "exclude",
  ): ReturnType<BillingRulePersistence["listAllBillingRules"]> {
    return new BillingRulePersistence(this.db).listAllBillingRules(enterpriseId, archived);
  }

  async createBillingRule(input: Parameters<BillingRulePersistence["createBillingRule"]>[0]) {
    return new BillingRulePersistence(this.db).createBillingRule(input);
  }
}
