...
describe("ZhipuAdapter", () => {
  ...
  it("企业自定义统一别名不被硬编码拒绝，并以资源 upstreamModel 为准", async () => {
    ...
    const adapter = new ZhipuAdapter(async (resource, request) => {
    ...
    req.unifiedModel = "enterprise-glm-alias";
    const resource = makeResource();
    resource.upstreamModel = "glm-vendor-custom";
    const outcome = await adapter.invoke(resource, req, 1);
    expect(outcome.status).toBe(200);
    expect(capturedRequest?.unifiedModel).toBe("enterprise-glm-alias");
    expect(capturedResource?.upstreamModel).toBe("glm-vendor-custom");
  });
