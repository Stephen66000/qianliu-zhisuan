...
it("正式数据库倍率解析驱动拒绝、逐次 6.60 元结算及不可变快照", async () => {
  ...
  stub = new StubUpstream({ default: { kind: "SUCCESS", usage: { input: 1000000, output: 100000, cache: 200000 } }, providerCode: "deepseek" });
  const app = await buildApp();
  try {
...
it.each(["SWITCH", "REJECT"] as const)("调用前策略变为 %s：重选或拒绝并冻结零消费证据", async (action) => {
  ...
  stub = new StubUpstream({ default: { kind: "SUCCESS", usage: { input: 100, output: 20, cache: 10 } }, providerCode: "deepseek" });
