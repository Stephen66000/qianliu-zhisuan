...
describe("Gateway 流式空闲超时策略", () => {
  it("默认仅将智谱 Coding Plan 放宽到 120 秒，其余厂商与智谱 API 保持 45 秒", () => {
    const policy = createStreamIdleTimeoutPolicy({});

    expect(policy(resource("deepseek", "API"))).toBe(45_000);
    expect(policy(resource("kimi", "CODING_PLAN"))).toBe(45_000);
    expect(policy(resource("zhipu", "API"))).toBe(45_000);
  ...
  it("按模式覆盖厂商值，厂商值覆盖全局值", () => {
    const policy = createStreamIdleTimeoutPolicy({
      ...
      GATEWAY_ZHIPU_STREAM_IDLE_TIMEOUT_MS: "90000",
      GATEWAY_ZHIPU_CODING_PLAN_STREAM_IDLE_TIMEOUT_MS: "130000",
    });

    expect(policy(resource("deepseek", "API"))).toBe(50_000);
    expect(policy(resource("zhipu", "API"))).toBe(90_000);
    expect(policy(resource("zhipu", "CODING_PLAN"))).toBe(130_000);
  });
  ...
  it.each(["deepseek", "zhipu", "kimi"] as const)(
    ...
    (provider) => {
      const prefix = `GATEWAY_${provider.toUpperCase()}`;
      const policy = createStreamIdleTimeoutPolicy({
        GATEWAY_UPSTREAM_STREAM_IDLE_TIMEOUT_MS: "40000",
        [`${prefix}_STREAM_IDLE_TIMEOUT_MS`]: "60000",
        [`${prefix}_API_STREAM_IDLE_TIMEOUT_MS`]: "70000",
        [`${prefix}_CODING_PLAN_STREAM_IDLE_TIMEOUT_MS`]: "80000",
      });
  ...
  it("空配置视为未配置并回退安全默认值", () => {
    const policy = createStreamIdleTimeoutPolicy({
      ...
      GATEWAY_ZHIPU_CODING_PLAN_STREAM_IDLE_TIMEOUT_MS: "",
    });
    expect(policy(resource("deepseek", "API"))).toBe(45_000);
    expect(policy(resource("zhipu", "CODING_PLAN"))).toBe(120_000);
  });
  ...
  it.each(["0", "-1", "1.5", "NaN", "9007199254740992"])(
    "启动时拒绝任意厂商或模式的无效空闲门限 %s",
    (value) => {
    expect(() => createStreamIdleTimeoutPolicy({
      GATEWAY_ZHIPU_CODING_PLAN_STREAM_IDLE_TIMEOUT_MS: value,
    })).toThrow("GATEWAY_ZHIPU_CODING_PLAN_STREAM_IDLE_TIMEOUT_MS 必须是正整数毫秒");
    },
  );
});
