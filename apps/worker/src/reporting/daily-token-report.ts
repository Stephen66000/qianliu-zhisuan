...
export function generateDailyReportSvg(data: DailyReportData): string {
  ...
  const rankingHeight = 60 + topUsersCount * 44;
  ...
  const userRowsSvg = data.topUsers
    .map((u, i) => {
      const y = 50 + i * 44;
      const barWidth = Math.max(4, Math.round((u.tokens / maxUserTokens) * 260));
      ...
      <g transform="translate(0, ${y})">
        ...
        <text x="44" y="20" font-size="14" font-weight="500" fill="#f8fafc">${escapeXml(u.name)}</text>
        <text x="160" y="20" font-size="12" fill="#94a3b8">${escapeXml(u.department || "-")}</text>
        
        <!-- Progress bar background -->
        <rect x="280" y="10" width="260" height="12" rx="6" fill="#334155" />
        <!-- Progress bar fill -->
        <rect x="280" y="10" width="${barWidth}" height="12" rx="6" fill="url(#barGrad)" />
        
        <text x="560" y="20" font-size="13" font-weight="600" fill="#38bdf8" text-anchor="start">${formatNumber(u.tokens)}</text>
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
    ...
    <g transform="translate(40, 36)">
      <text x="0" y="24" font-size="22" font-weight="bold" fill="#ffffff" letter-spacing="1">📊 仟流智算 · 全员 Token 消费日报</text>
      <text x="0" y="50" font-size="13" fill="#94a3b8">企业：${escapeXml(data.enterpriseName)}  |  日期：${escapeXml(data.reportDate)} (全天数据)</text>
      <text x="720" y="50" font-size="12" fill="#64748b" text-anchor="end">生成时间：${escapeXml(data.generatedAt)}</text>
    </g>
    ...
    <g transform="translate(40, 225)">
      ...
      <text x="20" y="30" font-size="15" font-weight="bold" fill="#f8fafc">🏆 员工用量消耗排行榜 Top ${data.topUsers.length}</text>
      <text x="200" y="30" font-size="12" fill="#64748b">部门</text>
      <text x="380" y="30" font-size="12" fill="#64748b">消耗进度</text>
      <text x="560" y="30" font-size="12" fill="#64748b">消耗 Tokens</text>
      <text x="700" y="30" font-size="12" fill="#64748b" text-anchor="end">占比</text>
      <line x1="16" y1="42" x2="704" y2="42" stroke="#334155" stroke-width="1" />

      ${userRowsSvg}
    </g>

    <!-- Model Distribution Section -->
    <g transform="translate(40, ${225 + rankingHeight + 20})">
      <rect x="0" y="0" width="720" height="${modelsHeight}" rx="10" fill="url(#cardGrad)" stroke="#334155" stroke-width="1" />
      <text x="20" y="30" font-size="15" font-weight="bold" fill="#f8fafc">🤖 主要模型消耗分布</text>
