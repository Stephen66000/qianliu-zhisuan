...
export async function loadResourceTokenUtilization(
  ...
  const result = await sql<{
    ...
    baseline_months: [string, string, string]; baseline_month_count: number;
    ...
    WITH bounds AS (
      ...
      SELECT ll.provider_resource_id,
             ...
             COALESCE(SUM(ll.raw_input_tokens::numeric + ll.raw_output_tokens)
               FILTER (WHERE COALESCE(ll.settled_at, ll.created_at) < b.started_at), 0) AS baseline_tokens
      ...
      SELECT pr.id AS resource_id, COALESCE(t.current_tokens, 0) AS current_tokens,
             ...
             ARRAY(SELECT to_char(b.month_start - n * interval '1 month', 'YYYY-MM')
                     FROM generate_series(3, 1, -1) AS n) AS baseline_months,
    ...
    SELECT resource_id, current_tokens::text, baseline_months, baseline_month_count,
           CASE WHEN baseline_month_count = 3 THEN round(baseline_tokens / 3, 8)::text END AS average_tokens,
           CASE WHEN baseline_month_count = 3 AND baseline_tokens > 0
             THEN round(current_tokens * 3 / baseline_tokens, 8)::text END AS rate,
           CASE WHEN baseline_month_count < 3 THEN 'INSUFFICIENT_HISTORY'
