...
const UsernameSchema = z.string().trim().min(1).max(128).regex(/^[A-Za-z0-9._-]+$/, {
  message: "用户名只能包含字母、数字、点、下划线和连字符",
});
const DisplayNameSchema = z.string().trim().min(1).max(128);
const PasswordSchema = z.string().max(128).refine(isStrongPassword, PASSWORD_POLICY_MESSAGE);
...
export function registerAdminRoutes(app: FastifyInstance): void {
  app.get("/admins", { preHandler: [requireAuth] }, async (req) => {
    const admins = await app.adminRepo.listByEnterprise(req.admin!.enterpriseId);
  ...
  app.post("/admins", { preHandler: [requireAuth] }, async (req, reply) => {
    ...
    try {
      ...
      const reason = isUniqueViolation(error) ? "duplicate_username" : "create_failed";
  ...
  app.patch("/admins/:id", { preHandler: [requireAuth] }, async (req, reply) => {
    const id = IdSchema.safeParse((req.params as { id?: unknown }).id);
    const body = RenameSchema.safeParse(req.body);
    if (!id.success || !body.success) {
      await auditFailure(app, req, "admin.rename", id.success ? id.data : null, "invalid_request");
    ...
    if (!admin) {
      ...
      return reply.code(404).send({ error: "not_found", message: "管理员不存在" });
    }
    await app.auditRepo.write({
      enterprise_id: req.admin!.enterpriseId,
      admin_user_id: req.admin!.adminUserId,
  ...
  app.post("/auth/change-password", { preHandler: [requireAuth] }, async (req, reply) => {
    ...
    if (!parsed.success) {
      await auditFailure(app, req, "admin.password.change", req.admin!.adminUserId, "invalid_request");
      return reply.code(400).send({
        error: "invalid_request",
        message: parsed.error.issues[0]?.message ?? "请求参数不合法",
      });
    ...
    if (!admin || !(await verifyPassword(admin.password_hash, parsed.data.current_password))) {
      await auditFailure(app, req, "admin.password.change", req.admin!.adminUserId, "invalid_current_password");
      return reply.code(400).send({
    ...
    if (!changed) {
      await auditFailure(app, req, "admin.password.change", admin.id, "concurrent_change");
      return reply.code(409).send({ error: "conflict", message: "密码已被其他会话修改，请重新登录" });
    }
    await app.auditRepo.write({
      enterprise_id: req.admin!.enterpriseId,
      admin_user_id: req.admin!.adminUserId,
  ...
  app.post("/admins/:id/reset-password", { preHandler: [requireAuth] }, async (req, reply) => {
    const id = IdSchema.safeParse((req.params as { id?: unknown }).id);
    const body = PasswordResetSchema.safeParse(req.body);
    if (!id.success || !body.success) {
      await auditFailure(app, req, "admin.password.reset", id.success ? id.data : null, "invalid_request");
      return reply.code(400).send({ error: "invalid_request", message: body.success ? "请求参数不合法" : (body.error.issues[0]?.message ?? "请求参数不合法") });
    }
    if (id.data === req.admin!.adminUserId) {
      await auditFailure(app, req, "admin.password.reset", id.data, "use_change_password");
      return reply.code(400).send({ error: "use_change_password", message: "请通过修改当前密码功能更新自己的密码" });
    }
    ...
    if (!admin) {
      await auditFailure(app, req, "admin.password.reset", id.data, "not_found");
      return reply.code(404).send({ error: "not_found", message: "管理员不存在" });
    }
    await app.auditRepo.write({
      enterprise_id: req.admin!.enterpriseId,
  ...
  for (const status of ["ACTIVE", "DISABLED"] as const) {
    ...
    app.post(`/admins/:id/${action}`, { preHandler: [requireAuth] }, async (req, reply) => {
      const id = IdSchema.safeParse((req.params as { id?: unknown }).id);
      if (!id.success) {
        await auditFailure(app, req, `admin.${action}`, null, "invalid_request");
        return reply.code(400).send({ error: "invalid_request", message: "管理员 ID 不合法" });
      }
      try {
        ...
        await app.auditRepo.write({
          ...
          target_type: "admin_user",
          target_id: admin.id,
          change_summary: { status: admin.status, sessions_revoked: status === "DISABLED" },
          result: "SUCCESS",
        });
        return { admin: publicAdmin(admin) };
      } catch (error) {
        const code = error instanceof SelfDisableError
