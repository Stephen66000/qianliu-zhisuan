  ...
  return (
    <div aria-labelledby="resource-tab-finance" id="resource-tab-panel-finance" role="tabpanel">
      ...
      <section className="mt-4 rounded-xl border border-ql-border-zone bg-ql-surface p-4">
        <div className="grid gap-3 md:grid-cols-2"><FormField htmlFor="finance-history-resource" label="历史记录资源"><select className={INPUT_CLASS} id="finance-history-resource" onChange={(event) => setResourceId(event.target.value || null)} value={resourceId ?? ""}><option value="">请选择</option>{resources.map((resource) => <option key={resource.id} value={resource.id}>{providerName(resource)} · {resource.name}</option>)}</select></FormField>{selected?.mode === "CODING_PLAN" ? <FinanceMetric label="当前服务周期" value={periods.data?.periods.find((period) => period.current_status === "ACTIVE") ? `${periods.data.periods.find((period) => period.current_status === "ACTIVE")!.period_start.slice(0, 10)} ～ ${periods.data.periods.find((period) => period.current_status === "ACTIVE")!.period_end_exclusive.slice(0, 10)} · ${formatCount(periods.data.periods.find((period) => period.current_status === "ACTIVE")!.token_usage.true_tokens)} Token` : "无有效周期"} /> : null}</div>
