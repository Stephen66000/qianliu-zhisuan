interface PrincipalKeyDialogProps {
  copyStatus: "idle" | "success" | "success-key" | "success-info" | "error";
  onClose: () => void;
  onCopy: () => void;
  onCopyKey?: () => void;
  open: boolean;
    ...
    >
      <div className="w-full max-w-lg rounded-2xl border border-ql-border bg-ql-surface-raised p-6 shadow-ql-raised">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          {props.onCopyKey ? (
            <button
              className="h-9 rounded-lg bg-ql-action px-4 text-[13px] font-medium text-white hover:opacity-90"
          ...
          <button
            className="h-9 rounded-lg border border-ql-action px-4 text-[13px] font-medium text-ql-action hover:bg-ql-action-soft"
          ...
          <button
            className="h-9 rounded-lg border border-ql-border px-4 text-[13px] font-medium text-ql-fg-secondary hover:bg-ql-surface-muted"
        ...
        {props.copyStatus === "success-key" ? (
          <p className="mt-2 text-right text-[12px] text-ql-success" role="status">
            Key 复制成功
          </p>
        ) : null}
        {props.copyStatus === "success-info" || props.copyStatus === "success" ? (
          <p className="mt-2 text-right text-[12px] text-ql-success" role="status">
            完整接入信息复制成功
          </p>
        ) : null}
        {props.copyStatus === "error" ? (
          <p className="mt-2 text-right text-[12px] text-ql-danger" role="alert">
            复制失败，请检查剪贴板权限
          </p>
        ) : null}
      </div>
