  ...
  return (
    <QueryGate emptyDescription="配置通讯录来源并执行同步，或上传标准模板。" emptyIcon={Users} emptyTitle="暂无通讯录成员" error={error} isEmpty={filteredRows.length === 0} isLoading={isLoading} onRetry={onRetry}>
      <div className="overflow-x-auto"><table className="w-full text-left text-[12px]"><thead><tr className="border-b border-ql-border text-ql-fg-tertiary">
        ...
        <th className="py-2">成员</th><th>员工编号</th><th>部门</th><th>来源</th><th>主体</th><th>接入配置</th><th className="text-right">操作</th></tr></thead><tbody>{filteredRows.map((member) => <tr className="border-b border-ql-border-zone" key={member.person_id}>
          ...
          <td className="font-medium">{member.name}</td><td>{member.employee_number ?? "—"}</td><td>{member.department_name ?? "待归属"}</td><td>{member.source_type ?? "手工"}</td><td>{member.principal_status ?? "未建立"}</td><td>{member.access_config_status}</td>
