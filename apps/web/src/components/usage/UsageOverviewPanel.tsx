...
export function UsageOverviewPanel() {
  ...
  return <div className="space-y-3">
    <div className="rounded-lg border border-ql-border-zone bg-ql-surface-subtle p-3">
      <div className="grid grid-cols-2 items-start gap-2 lg:grid-cols-[88px_88px_148px_minmax(0,1fr)]">
        ...
        <select aria-label="用量周期" className={usageInputClass} onChange={(event) => set("period", event.target.value)} value={period}><option value="TODAY">今日</option><option value="WEEK">本周</option><option value="MONTH">本月</option></select>
        <input aria-label="用量锚点" className={`${usageInputClass} col-span-2 lg:col-span-1`} onChange={(event) => { if (event.target.value) set("anchor", new Date(`${event.target.value}T12:00:00+08:00`).toISOString()); }} type="date" value={anchor.slice(0, 10)}/>
