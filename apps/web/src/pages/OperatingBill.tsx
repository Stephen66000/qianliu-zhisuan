...
function Overview({ bill }: { bill: OperatingBill }) {
  ...
  const metrics = [
    ["期初余额", money(bill.summary.openingBalance ?? null), bill.summary.openingBalance === null ? "待补期初余额" : "月初有效快照或上月期末承接"],
    ["本月充值", money(bill.summary.monthlyRecharge ?? null), "本账期 API 充值现金"],
    ["期末余额", money(bill.summary.endingBalance), bill.summary.endingBalance === null ? "待补期末余额" : (bill.summary.endingBalanceCurrency ?? "API 预付余额")],
    ["API 花费", money(bill.summary.apiCost), bill.summary.apiSpendReason ?? "期初余额 + 本月充值 - 期末余额"],
    ["套餐费用", money(bill.summary.packageCost), "当月固定套餐成本"],
    ["本月总花费", money(bill.summary.totalCost), bill.summary.totalCost === null ? (bill.summary.apiSpendReason ?? (bill.summary.packageCost === null ? "待补套餐费用" : "月度总花费不可计算")) : "API 花费 + 固定套餐费用"],
...
function OpeningBalanceEntry({ bill }: { bill: OperatingBill }) {
  ...
  const [currency, setCurrency] = useState(
    selected?.currency ?? bill.summary.endingBalanceCurrency ?? "CNY",
  );
  ...
  const save = () => {
    ...
    if (!resourceId) { setValidationError("请选择 API 资源"); return; }
    setValidationError("");
    mutation.mutate({
  ...
  return <BillCard><SectionHeading title="补录期初余额" description="仅补充当前账期的期初事实；不覆盖资源当前余额，保存后自动重算月度经营结果"/><div className="grid gap-3 px-4 pb-4 md:grid-cols-4"><select aria-label="期初余额资源" className={inputClass} disabled={mutation.isPending} onChange={event => { const next = resources.find(row => row.providerResourceId === event.target.value); setResourceId(event.target.value); if (next?.currency) setCurrency(next.currency); }} value={resourceId}>{resources.map(row => <option key={row.providerResourceId} value={row.providerResourceId}>{row.providerName} · {row.resourceName}</option>)}</select><MoneyAmountInput aria-invalid={Boolean(validationError)} className={inputClass} disabled={mutation.isPending} id="opening-balance-amount" onChange={setAmount} placeholder="期初余额" value={amount}/><input aria-label="期初余额币种" className={inputClass} disabled={mutation.isPending} maxLength={8} onChange={event => setCurrency(event.target.value.toUpperCase())} placeholder="CNY" value={currency}/><input aria-label="期初余额说明" className={inputClass} disabled={mutation.isPending} maxLength={1000} onChange={event => setReason(event.target.value)} placeholder="补录依据（可选）" value={reason}/><div className="md:col-span-4 flex items-center justify-end gap-3">{validationError ? <span className="text-[12px] text-ql-danger" role="alert">{validationError}</span> : null}{mutation.error ? <span className="text-[12px] text-ql-danger">{mutation.error.message}</span> : null}<button className={buttonPrimary} disabled={mutation.isPending} onClick={save} type="button">保存并重算</button></div></div></BillCard>;
}
  ...
  return <div className="space-y-4">
    <BillCard className="overflow-hidden">
      ...
      <div className="grid gap-3 border-b border-ql-border-zone p-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <ReviewMetric label="API 花费" value={money(review.resources.reduce((sum, row) => sum + Number(row.apiCost), 0).toFixed(8))}/>
      ...
      <Table headers={["资源", "形态", "采购/充值现金", "真实使用", "本月花费", "利用率 / 口径", "耗尽 / 连续无调用", "建议标签", "确定性依据"]}>{review.resources.map((row) => <tr className="border-b border-ql-border-zone align-top" key={row.resourceId}>
