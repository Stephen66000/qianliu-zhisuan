...
function ResourceFactConfirmation({ bill, provider }: { bill: OperatingBill; provider: OperatingBill["providers"][number] }) {
  ...
  return <tr className="border-b border-ql-border-zone"><Cell>{provider.resourceName}<span className="block text-[11px] text-ql-fg-tertiary">{provider.mode}</span></Cell><Cell>{purchaseText}</Cell><Num>{provider.mode === "CODING_PLAN" ? money(provider.packageCost) : "不适用"}</Num><Num>{provider.mode === "API" ? money(provider.apiCost) : "套餐内"}</Num><Num>{provider.mode === "API" ? money(provider.endingBalance) : "不适用"}</Num><Cell>{provider.servicePeriodStart || provider.servicePeriodEnd ? `${provider.servicePeriodStart ?? "未知"} ~ ${provider.servicePeriodEnd ?? "未知"}` : "未提供"}</Cell><Cell>{provider.operatingSnapshotSource ?? "未知来源"}<span className="block text-[11px] text-ql-fg-tertiary">{provider.operatingSnapshotVersion ? `快照 v${provider.operatingSnapshotVersion} · ${provider.operatingSnapshotAt ?? "时间未知"}` : "缺少快照"}</span><span className="block text-[11px] text-ql-fg-tertiary">请求 ${provider.requestRange?.count ?? 0} 条</span></Cell><Cell><select aria-label={`${provider.resourceName} 确认状态`} className={inputClass} disabled={bill.status === "CLOSED" || mutation.isPending} onChange={event => setStatus(event.target.value as typeof status)} value={status}><option value="PENDING">待补充</option><option value="CONFIRMED">属实</option><option value="ANOMALY">异常</option><option value="NOT_APPLICABLE">不适用</option></select><input aria-label={`${provider.resourceName} 确认备注`} className={`${inputClass} mt-1`} onChange={event => setNote(event.target.value)} placeholder="备注" value={note}/><button className="mt-1 text-ql-action" disabled={bill.status === "CLOSED" || mutation.isPending || ((status === "ANOMALY" || status === "NOT_APPLICABLE") && !note.trim())} onClick={() => mutation.mutate({ status, note: note.trim() || null })}>保存确认</button>{provider.confirmation?.confirmedBy ? <span className="block text-[11px] text-ql-fg-tertiary">{provider.confirmation.confirmedBy} · {provider.confirmation.confirmedAt ? new Date(provider.confirmation.confirmedAt).toLocaleString("zh-CN") : ""}</span> : null}</Cell></tr>;
}
