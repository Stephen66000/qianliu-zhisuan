...
function NotificationsPanel() {
  ...
  const bindMutation = useMutation({ mutationFn: () => { const principal = (principalsQuery.data?.principals ?? []).find((item) => item.id === selectedPrincipal); if (!principal) throw new Error("请选择主体"); return patch(`/principals/${principal.id}/${principal.type === "EMPLOYEE" ? "person" : "owner"}`, { person_id: selectedPerson, expected_version: principal.version }); }, onSuccess: refresh });
