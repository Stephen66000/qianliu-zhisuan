...
3. **极简纯白长图与移动端留白缩减 50% 规范**：
   - **纯白底色**：全量移除原先外围的 `#EEF2F6` 灰蓝衬底画板与卡片浮动阴影，卡片本身即一张纯白长图（底色全量统一为 `#FFFFFF`）；
   - **四周留白减少 50%**：针对移动端屏幕阅读优化，将四周多余留白减少 50%，核心文字等比放大，手机屏单手查阅清晰舒展（个人/激励卡片 `viewBox="47 65 446 635"`，团队看板 `viewBox="44 60 452 680"`）；
   - **官方 Logo 蓝角与英文水平绝对对齐法则**：对齐基准不是 Logo 整体图片外框，而是 **Logo 蓝色的最下端（即右下角蓝色括弧 `┘` 的最底边缘），必须与左侧英文字样「Qianliu IC」整个英文的最下端（Baseline）处于同一条绝对水平线上**。
4. **Emoji 避坑与模型名称清洗原则**：
   - 由于 `@resvg/resvg-js` 服务端渲染环境缺失彩色 Emoji 字体，若在 SVG 中直接写 `🥇`、`🚀` 会被渲染为乱码或方框（`????`）。卡片中所有徽章、奖牌一律采用矢量图形（如 `<circle>`、圆角矩形）绘制；
   - 模型展示名称统一去除 `ql-` 等内部技术前缀，规范呈现业务友好名称（如 `DeepSeek V3 + K3`）。
...
- [x] 1. 生成的所有图片均为纯白底色（`#FFFFFF`）长图，无外围灰蓝画板与浮动阴影；
- [x] 2. 四周外层留白减少 50%，核心文字与排版在移动端等比放大 25%，手机单手阅读大方舒展；
- [x] 3. 官方 Logo 蓝角最下端（右下角蓝色括弧 `┘` 最底像素）与左侧「Qianliu IC」英文字样最下端（Baseline）处于同一条水平线上，严禁偏高或偏低；
