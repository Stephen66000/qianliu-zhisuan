#!/usr/bin/env bash
# 额度规则优化：从 7b27014 发布计价与调度修复，执行兼容增量迁移 0064。
...
if test "${1:-}" = "--check-contract"; then
  test "$#" = 1
  bash -n "$0"
  grep -q 'candidate_ref="refs/heads/codex/quota-pricing-review-20260905"' "$0"
  grep -q 'source_commit="7b2701442c2bd5e83709c6282d549d4a5a581cbe"' "$0"
...
candidate_ref="refs/heads/codex/quota-pricing-review-20260905"
candidate_commit="${CANDIDATE_COMMIT:?请传入已审核候选的完整 Commit SHA}"
candidate_tree="${CANDIDATE_TREE:?请传入已审核候选的完整 Tree SHA}"
source_commit="7b2701442c2bd5e83709c6282d549d4a5a581cbe"
expected_migration="0063_operating_snapshot_subscription_period"
target_migration="0064_quota_pricing_and_policy_archive"
...
release="${server_root}/releases/qianliu-quota-pricing-${short}-${stamp}"
backup_dir="${server_root}/backups/qianliu-zhisuan"
backup="${backup_dir}/pre-quota-pricing-${short}-${stamp}.dump"
log_dir="${server_root}/logs/qianliu-zhisuan"
log_file="${log_dir}/deploy-quota-pricing-${short}-${stamp}.log"
lock_dir="${server_root}/.qianliu-quota-pricing-release.lock"

if ! mkdir "$lock_dir" 2>/dev/null; then
  echo "已有额度规则发布任务持有锁: $lock_dir" >&2
...
rollback() {
  ...
  if test "$paused" = 1; then
    if ! stop_business; then
      echo "ERROR: 无法确认停写，暂停自动回退，请人工处理" >&2
      return
    fi
    if ! pricing_column="$(db_query "SELECT COUNT(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='billing_rule' AND column_name='pricing_mode';")"; then
      echo "ERROR: 数据库结构查询失败，保持停写，暂停自动回退" >&2
      return
    fi
    if test "$pricing_column" != 0 && test "$pricing_column" != 1; then
      echo "ERROR: 无法确认数据库计价结构，保持停写，暂停自动回退" >&2
      return
    fi
    if test "$pricing_column" = 1; then
      if ! multiplier_rules="$(db_query "SELECT COUNT(*) FROM billing_rule WHERE pricing_mode='MULTIPLIER';")"; then
        echo "ERROR: 倍率规则查询失败，保持停写，暂停自动回退" >&2
        return
      fi
      if test "$multiplier_rules" != 0; then
        echo "ERROR: 已存在新倍率规则或无法确认；旧程序不能正确结算，保持业务停止。保留数据库及备份，需修复前滚。" >&2
        return
      fi
    fi
  fi
  ...
  if test "$images_frozen" = 1; then
    for service in control-api gateway worker web; do
      docker image tag "qianliu-quota-pricing-rollback-${short}-${service}" \
  ...
  if test "$paused" = 1 && test -n "$previous"; then
    ...
    for service in control-api gateway worker web; do
      ...
      test "$(docker inspect --format '{{.Image}}' "$container" 2>/dev/null)" = \
        "$(docker image inspect --format '{{.Id}}' "qianliu-quota-pricing-rollback-${short}-${service}" 2>/dev/null)" \
...
log "step 2: verify single additive migration and unchanged deployment topology"
git -C "$release" diff --quiet "$source_commit..$candidate_commit" -- \
  packages/database/migrations ':(exclude)packages/database/migrations/0064_quota_pricing_and_policy_archive.js' deploy/compose.yaml deploy/caddy/Caddyfile deploy/postgres-init
...
unexpected_migrations="$(find "$release/packages/database/migrations" -maxdepth 1 -type f -name '*.js' \
  -exec basename {} \; | awk -F_ '$1 ~ /^[0-9]+$/ && ($1 + 0) > 64 {print}')"
test -z "$unexpected_migrations"
test -f "$release/packages/database/migrations/0064_quota_pricing_and_policy_archive.js"
...
for service in control-api gateway worker web; do
  ...
  docker image tag "$current_image" "qianliu-quota-pricing-rollback-${short}-${service}"
done
images_frozen=1
...
log "step 6: apply additive migration, then start candidate"
(cd "$release/deploy" && docker compose run --rm --no-deps migrate)
test "$(db_query 'SELECT name FROM kysely_migration ORDER BY timestamp DESC LIMIT 1;')" = "$target_migration"
...
log "COMPLETE release=${release} commit=${candidate_commit} tree=${candidate_tree} database=${target_migration} control=${control_code} gateway=${gateway_code} web=${web_code} caddy=${caddy_code} worker=${worker_health} backup=${backup} backup_sha256=${backup_sha}"
