...
candidate=72456694118f42c51f199c95a24e665bc7d25fca
tree=7b6f1e690bfdf305defbf15ae60cbe5625807cb4
expected_source=eb1ed123ec8e919cad0e78947c9cd9c441dee352
source_tree=bb364298d362c22c7628b7f0d36e160563b0deaf
migration=0070_alert_recovery_evidence
intermediate_migration=0071_enterprise_contact_details
target_migration=0072_admin_roles_security
root=/Users/stephen
...
case "$mode" in
  ...
  --preflight|--deploy) ;;
  *) echo 'Usage: bash release-system-settings-20260909-mac-mini.sh [--preflight|--deploy|--check-contract]' >&2; exit 2 ;;
esac
...
intermediate_names="$(printf '%s\n%s' "$source_names" "$intermediate_migration")"
target_names="$(printf '%s\n%s' "$intermediate_names" "$target_migration")"
db_sql() {
...
safe_previous() {
  local names settings_count
  names="$(db_names)" || return 1
  if test "$names" = "$source_names" || test "$names" = "$intermediate_names"; then return 0; fi
  test "$names" = "$target_names" || return 1
  # The source grants every admin full access. New roles or a changed security policy
  # prohibit restoring those old writers, even though the schema is additive.
  settings_count="$(db_sql "SELECT (SELECT COUNT(*) FROM admin_role) + (SELECT COUNT(*) FROM admin_user WHERE role_code = 'CUSTOM') + (SELECT COUNT(*) FROM enterprise WHERE security_version > 1);")" || return 1
  test "$settings_count" = 0
}
...
on_exit() {
  ...
  if test "$success" != 1; then
    ...
    if test "$rollback_ok" != 1; then
      ...
      echo "STOP: recovery cannot be verified or new role/security settings exist. Database/backup preserved; lock retained at $lock. Forward repair required." >&2
      exit "$status"
    fi
...
release="$(mktemp -d "$root/releases/qianliu-system-settings-$stamp.XXXXXX")"
backup="$root/backups/qianliu-zhisuan/pre-system-settings-$stamp.dump"
log="$root/logs/qianliu-zhisuan/deploy-system-settings-$stamp.log"
rollback_prefix="qianliu-system-settings-rollback-$stamp"
exec > >(tee -a "$log") 2>&1
...
test "$(git -C "$release" -c diff.renames=false diff --name-status "$expected_source..$candidate" -- packages/database/migrations)" = $'A\tpackages/database/migrations/0071_enterprise_contact_details.js\nA\tpackages/database/migrations/0072_admin_roles_security.js'
...
test "$(shasum -a 256 "$release/packages/database/migrations/0071_enterprise_contact_details.js" | awk '{print $1}')" = df71891f6c6aaad30334392fd0ebf851bbec4a3ebc7955087798e781ae655f9f
test "$(shasum -a 256 "$release/packages/database/migrations/0072_admin_roles_security.js" | awk '{print $1}')" = 74c599fc8180c93f8dc9678f274dae077b0669a907db5125c6abd6250be7b90a
...
# Stamp only the new release; preserve the source environment for recovery.
awk '!/^[[:space:]]*(export[[:space:]]+)?APP_VERSION[[:space:]]*=/' "$release/deploy/.env" > "$release/deploy/.env.next"
printf '\nAPP_VERSION=%s\n' "$candidate" >> "$release/deploy/.env.next"
