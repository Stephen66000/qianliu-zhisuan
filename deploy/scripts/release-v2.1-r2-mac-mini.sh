...
if test "${1:-}" = "--check-contract"; then
  ...
  grep -q 'target_head="0054_usage_aggregate_settlement_time"' "$0"
  grep -q '0052_dispatch_restore_and_resource_utilization' "$0"
  grep -q '0053_operating_bill_opening_balance.js' "$0"
  grep -q '0054_usage_aggregate_settlement_time.js' "$0"
...
target_head="0054_usage_aggregate_settlement_time"
server_home="${QIANLIU_SERVER_HOME:-/Users/stephen}"
product_commit="d315883a22e69496771fb15648226b44633a5dc2"
product_tree="eced60bd6fc3b31cb34d0827d075e383d2a27532"
...
release="${server_home}/releases/qianliu-zhisuan-v2.1-r2-${candidate_short}-${stamp}"
current_pointer="${server_home}/qianliu-current-release.txt"
backup_dir="${server_home}/backups/qianliu-zhisuan"
backup="${backup_dir}/pre-v2.1-r2-${stamp}.dump"
log_dir="${server_home}/logs/qianliu-zhisuan"
log_file="${log_dir}/deploy-v2.1-r2-${candidate_short}-${stamp}.log"
lock_dir="${server_home}/.qianliu-v2.1-r2-release.lock"
...
handle_failure() {
  ...
  if test "$images_frozen" = 1; then
    for service in control-api gateway worker web; do
      docker image tag "qianliu-v21r2-rollback-${candidate_short}-${service}" \
  ...
  if test "$paused" = 1; then
    ...
    if test "$migration_started" = 0 || test "$current_head" = "$source_head"; then
      ...
      log "restore step 4: for service in control-api gateway worker web; do docker image tag \"qianliu-v21r2-rollback-${candidate_short}-\$service\" \"qianliu-zhisuan-\$service\"; done"
...
unexpected_migrations="$(find "$release/packages/database/migrations" -maxdepth 1 -type f -name '*.js' \
  -exec basename {} \; | awk -F_ '$1 ~ /^[0-9]+$/ && ($1 + 0) > 54 {print}')"
...
for service in control-api gateway worker web; do
  ...
  test -n "$old_id"
  docker image tag "$old_id" "qianliu-v21r2-rollback-${candidate_short}-${service}"
done
images_frozen=1
...
test "$schema_check" = "1,5,1"
log "migration evidence 0053_sha=${migration_0053_sha} 0054_sha=${migration_0054_sha}"
log "rollback boundary: 0053 facts and 0054 MIXED facts may reject down; restore verified backup instead"
