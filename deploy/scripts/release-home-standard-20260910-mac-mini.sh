#!/usr/bin/env bash
# Standard homepage update on production e96e644; preserves migration 0073 and Chat recovery.
set -Eeuo pipefail
umask 077
...
test "$#" = 1 && test "$1" = --deploy || { echo "Usage: CANDIDATE_COMMIT=... CANDIDATE_TREE=... bash $0 --deploy"; exit 2; }
candidate="${CANDIDATE_COMMIT:?missing CANDIDATE_COMMIT}"
tree="${CANDIDATE_TREE:?missing CANDIDATE_TREE}"
source_commit=e96e6446d214294ca6b567e7fcd3d4d437c763c4
root=/Users/stephen
...
release="$root/releases/qianliu-home-standard-${candidate:0:7}-$stamp"
backup_image="qianliu-home-standard-rollback-$stamp"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-home-standard-$stamp.log") 2>&1
...
GIT_SSH_COMMAND='ssh -o BatchMode=yes' GIT_TERMINAL_PROMPT=0 git clone --depth 8 \
  --branch codex/home-standard-release-20260910 git@github.com:Stephen66000/qianliu-zhisuan.git "$release"
...
# This release must not change migrations, runtime topology or credential recovery.
git -C "$release" diff --quiet "$source_commit..$candidate" -- \
  packages/database/migrations deploy/compose.yaml deploy/compose.target.yaml \
  apps/gateway apps/worker packages/domain packages/provider-adapters
