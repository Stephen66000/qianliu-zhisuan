#!/usr/bin/env bash
# 从已核对的生产 0068 原位升级凭证隔离恢复规则与 401 脱敏证据。默认只预检。
set -Eeuo pipefail
umask 077
export PATH="${PATH}:/usr/local/bin:/opt/homebrew/bin:/Applications/Docker.app/Contents/Resources/bin"

candidate=1dc468369c56fb59f3a64d40b6136d102b5a1a41
candidate_tree=3beaa4deb0d0da808b6ed4592d08baeb5ca9ef69
expected_source=0bf4c1c0a17afc3a4a380f92958297efd46026b6
source_tree=468182ad84d860aa73b0b71968bfe0453410c4c2
source_migration=0068_alert_resource_context
target_migration=0069_auth_error_evidence
migration_sha=193ac8628b278b150abcd9ce15795a8d936bd2b7e5caeb846f67e31f855d68b6
server_root=/Users/stephen
pointer="$server_root/qianliu-current-release.txt"
lock="$server_root/.qianliu-credential-recovery-release.lock"
...
on_exit() {
  ...
  if test "$success" != 1; then
    ...
    if test "$started" = 1; then stop_and_verify || rollback_ok=0; fi
    ...
    echo "FAILED: 已恢复上一版应用；0069 为兼容性增量迁移，可能保留" >&2
  fi
...
release="$(mktemp -d "$server_root/releases/qianliu-credential-recovery-$stamp.XXXXXX")"
backup="$server_root/backups/qianliu-zhisuan/pre-credential-recovery-$stamp.dump"
log="$server_root/logs/qianliu-zhisuan/deploy-credential-recovery-$stamp.log"
rollback_prefix="qianliu-credential-recovery-rollback-$stamp"
exec > >(tee -a "$log") 2>&1
...
test "$(git -C "$release" -c diff.renames=false diff --name-status "$expected_source..$candidate" -- packages/database/migrations)" = \
  $'A\tpackages/database/migrations/0069_auth_error_evidence.js'
...
test "$(shasum -a 256 "$release/packages/database/migrations/0069_auth_error_evidence.js" | awk '{print $1}')" = "$migration_sha"
