#!/usr/bin/env bash
# Operating analysis partial-usage fix. No migration, no dependency or topology changes.
set -Eeuo pipefail
...
mode="${1:-deploy}"
case "$mode" in deploy|--preflight) ;; *) echo 'Usage: bash release-operating-partial-20260907-mac-mini.sh [--preflight|--check-contract]'; exit 2;; esac
test "$#" -le 1
candidate=ce09d97b5374dd9d70eff826174297d2fb171754
tree=3cd02ec1a0643f461d2572c698d927134b7d239b
...
release="$root/releases/qianliu-partial-ce09d97-$stamp"
backup="$root/backups/qianliu-zhisuan/pre-partial-ce09d97-$stamp.dump"
rollback_prefix="qianliu-partial-ce09d97-rollback-$stamp"
log="$root/logs/qianliu-zhisuan/deploy-partial-ce09d97-$stamp.log"
started=0
frozen=0
pointer_changed=0
success=0
...
while IFS= read -r changed; do
  case "$changed" in
    V4/Evidence/OPERATING-BILL-20260906/PARTIAL-USAGE-FIX.md|V4/Evidence/OPERATING-BILL-20260906/RELEASE-PREP.md|apps/web/src/api/operating-analysis.ts|apps/web/src/components/operating-bill/OperatingAnalysisPartial.test.tsx|apps/web/src/components/operating-bill/OperatingPlans.tsx|apps/web/src/components/operating-bill/OperatingProcurement.tsx|apps/web/src/components/operating-bill/OperatingTrends.tsx|apps/web/src/pages/OperatingBill.tsx|apps/web/src/pages/OperatingBillOverview.tsx|deploy/scripts/release-operating-bill-20260907-mac-mini.sh|packages/database/src/__tests-integration__/operating-analysis.integration.test.ts|packages/database/src/repositories/operating-analysis-repository.ts|packages/database/src/repositories/operating-analysis-usage.ts) ;;
