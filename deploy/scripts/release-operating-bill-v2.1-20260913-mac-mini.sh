#!/usr/bin/env bash
# Operating bill v2.1 release on production Mac mini.
# Includes financial continuous ledger, value realization (account multiplexing & plan dilution),
# restored department bill, dynamic multi-vendor columns, and CSV export.
...
release="$root/releases/qianliu-operating-bill-v2.1-$stamp"
backup_image="qianliu-bill-v2.1-rollback-$stamp"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-operating-bill-v2.1-$stamp.log") 2>&1

echo "=========================================="
echo "开始部署: 仟流智算经营账单 v2.1 (分支: $branch)"
echo "时间: $stamp"
echo "=========================================="
...
if ! grep -q '^FEATURE_DEPARTMENT_COST=' "$release/deploy/.env"; then
  echo 'FEATURE_DEPARTMENT_COST=true' >> "$release/deploy/.env"
else
  sed -i '' 's/^FEATURE_DEPARTMENT_COST=.*/FEATURE_DEPARTMENT_COST=true/' "$release/deploy/.env"
fi
...
echo "经营账单特性: FEATURE_DEPARTMENT_COST=true"
echo "资金模式: PROVIDER_FINANCE_MODE=ACTIVE"
echo "=========================================="
