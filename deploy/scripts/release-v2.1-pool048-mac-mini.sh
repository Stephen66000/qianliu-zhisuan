#!/usr/bin/env bash
# Mac Mini 原位发布：从已部署 2.1 r2 / 0054 升级 POOL20-048 / 0055。
...
product_commit="e2d64e93d5bd47612c162035796610358b6def2d"
product_tree="c35c39e1ab7a9f5cfd0b653558b47ce6901d4bab"
...
release="${server_home}/releases/qianliu-zhisuan-v2.1-pool048-${candidate_short}-${stamp}"
current_pointer="${server_home}/qianliu-current-release.txt"
backup_dir="${server_home}/backups/qianliu-zhisuan"
backup="${backup_dir}/pre-v2.1-pool048-${stamp}.dump"
log_dir="${server_home}/logs/qianliu-zhisuan"
log_file="${log_dir}/deploy-v2.1-pool048-${candidate_short}-${stamp}.log"
lock_dir="${server_home}/.qianliu-v2.1-pool048-release.lock"
...
unexpected_migrations="$(find "$release/packages/database/migrations" -maxdepth 1 -type f -name '*.js' \
  -exec basename {} \; | awk -F_ '$1 ~ /^[0-9]+$/ && ($1 + 0) > 55 {print}')"
...
test "$source_head" = "0054_usage_aggregate_settlement_time" \
  || { log "unsupported source migration=${source_head}; expected 0054"; exit 2; }
