#!/usr/bin/env bash
# Runtime assurance notification recipients and 0074 migration release on production Mac mini.
# Migrates database from 0073_credential_chat_probe to 0074_runtime_notification_recipients.
set -Eeuo pipefail
umask 077
...
if test "$mode" = --preflight; then
  ...
  echo "当前运行版本: $previous"
  echo "当前数据库版本: $current_db"
  echo "健康状态: 正常"
  echo "目标迁移版本: 0074_runtime_notification_recipients"
  echo "=========================================="
  exit 0
fi

stamp="$(date '+%Y%m%d-%H%M%S')"
release="$root/releases/qianliu-runtime-notification-$stamp"
backup_image="qianliu-rt-notif-rollback-$stamp"
backup_dump="$root/backups/qianliu-zhisuan/pre-runtime-notification-$stamp.dump"
lock="$root/.qianliu-release.lock"

mkdir "$lock" 2>/dev/null || {
  echo "发布锁已存在或被占用，请确认是否有其他发布正在执行: $lock"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-runtime-notification-$stamp.log") 2>&1

echo "=========================================="
echo "开始部署: 仟流智算运行保障通知人员与告警策略 (分支: $branch)"
echo "时间: $stamp"
echo "=========================================="
...
echo 'step 3: 构建新版本容器镜像 (migrate, control-api, gateway, worker, web)'
(cd "$release/deploy" && docker compose build migrate control-api gateway worker web)

echo 'step 4: 数据库安全备份与执行迁移'
if test "$(db_head)" = 0073_credential_chat_probe; then
  echo "创建数据库快照: $backup_dump"
  docker compose --project-directory "$previous/deploy" exec -T postgres \
    sh -lc 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc' > "$backup_dump"
  test -s "$backup_dump"
  docker compose --project-directory "$previous/deploy" exec -T postgres \
    sh -lc 'pg_restore -l >/dev/null' < "$backup_dump"
  echo "数据库快照验证成功"

  echo "运行迁移: 0073_credential_chat_probe -> 0074_runtime_notification_recipients"
  (cd "$release/deploy" && docker compose run --rm --no-deps migrate)
else
  echo "数据库基线已为 $(db_head)，跳过重复迁移"
fi

db_after="$(db_head)"
echo "迁移后数据库版本: $db_after"
test "$db_after" = 0074_runtime_notification_recipients

table_check="$(docker compose --project-directory "$previous/deploy" exec -T postgres sh -lc \
  'psql -X -v ON_ERROR_STOP=1 -Atq -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT count(*) FROM information_schema.tables WHERE table_name = '\''runtime_notification_recipient'\'';"' || true)"
test "$table_check" = "1"
echo "runtime_notification_recipient 表存在性验证成功"

echo 'step 5: 切换并拉起新版本容器'
test "$(<"$pointer")" = "$previous"
started=1
(cd "$release/deploy" && docker compose up -d --no-build --force-recreate --no-deps "${services[@]}")

echo 'step 6: 验证容器拓扑与健康检查'
...
echo "数据库版本: 0074_runtime_notification_recipients"
echo "服务状态: control=200 gateway=200 web=200 caddy=200 worker=healthy"
echo "回滚镜像备份: $backup_image"
test -f "$backup_dump" && echo "数据库备份快照: $backup_dump"
echo "=========================================="
