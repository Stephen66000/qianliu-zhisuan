#!/usr/bin/env bash
# DeepSeek version metadata update on the deployed system-settings base; no migration.
set -Eeuo pipefail
umask 077
...
source_commit=72456694118f42c51f199c95a24e665bc7d25fca
root=/Users/stephen
...
release="$root/releases/qianliu-deepseek-version-${candidate:0:7}-$stamp"
backup_image="qianliu-deepseek-version-rollback-$stamp"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-deepseek-version-$stamp.log") 2>&1
...
GIT_SSH_COMMAND='ssh -o BatchMode=yes' GIT_TERMINAL_PROMPT=0 git clone --depth 8 \
  --branch codex/deepseek-version-release-20260910 git@github.com:Stephen66000/qianliu-zhisuan.git "$release"
...
# Only reviewed version metadata, display, tests and this release helper may differ.
git -C "$release" diff --quiet "$source_commit..$candidate" -- . \
  ':(exclude)packages/provider-adapters/src/model-discovery.ts' \
  ':(exclude)packages/provider-adapters/src/model-discovery-contract.ts' \
  ':(exclude)packages/provider-adapters/src/deepseek-model-version.ts' \
  ':(exclude)packages/provider-adapters/src/__tests__/deepseek-model-version.test.ts' \
  ':(exclude)packages/provider-adapters/src/__tests__/model-discovery.test.ts' \
  ':(exclude)apps/web/src/components/resources/ResourceModelDiscovery.tsx' \
  ':(exclude)apps/web/src/components/resources/ResourceModelDiscovery.version.test.tsx' \
  ':(exclude)apps/control-api/src/__tests-integration__/deepseek-version-sync.test.ts' \
  ':(exclude)deploy/scripts/release-deepseek-version-20260910-mac-mini.sh'
