#!/usr/bin/env bash
# Web-only source update; keep database at 0064 and reuse unchanged backend images.
set -Eeuo pipefail
umask 077
...
source_commit=bd64d92e889bb4c391ea89bb3ec0aff1de43972a
root=/Users/stephen
...
release="$root/releases/qianliu-quota-ui-${candidate:0:7}-$stamp"
backup_image="qianliu-quota-ui-rollback-$stamp"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-quota-ui-$stamp.log") 2>&1
...
test "$(db_head)" = 0064_quota_pricing_and_policy_archive
echo 'step 1: fetch and verify exact candidate'
GIT_SSH_COMMAND='ssh -o BatchMode=yes' GIT_TERMINAL_PROMPT=0 git clone --depth 8 \
  --branch codex/quota-pricing-review-20260905 git@github.com:Stephen66000/qianliu-zhisuan.git "$release"
...
git -C "$release" diff --quiet "$source_commit..$candidate" -- . \
  ...
  ':(exclude)apps/web/src/components/quota/QuotaPolicySection.tsx' \
  ':(exclude)apps/web/src/pages/QuotaRules.tsx' \
  ':(exclude)apps/web/src/pages/QuotaPricingWorkflow.test.tsx' \
  ':(exclude)apps/gateway/src/__tests-integration__/peak-calendar-settlement.test.ts' \
  ':(exclude)deploy/scripts/release-quota-ui-20260906-mac-mini.sh'
...
success=1
echo "COMPLETE release=$release commit=$candidate tree=$tree database=0064_quota_pricing_and_policy_archive control=200 gateway=200 web=200 caddy=200 worker=healthy rollback_image=$backup_image"
