#!/usr/bin/env bash
# August subscription cost projection and compact amount display. No schema or dependency changes.
set -Eeuo pipefail
...
mode="${1:-deploy}"
case "$mode" in deploy|--preflight) ;; *) echo 'Usage: bash release-august-overview-20260907-mac-mini.sh [--preflight|--check-contract]'; exit 2;; esac
test "$#" -le 1
candidate=feca8603210bcc109346faf03860bfea50d61742
tree=82d1a4f1553fc362eecaaf18dfbf4c38348687c6
expected_source=de0a2ec3b7446ab3e0b6d81344d7d84ac2163eab
...
release="$root/releases/qianliu-august-overview-feca860-$stamp"
backup="$root/backups/qianliu-zhisuan/pre-august-overview-feca860-$stamp.dump"
rollback_prefix="qianliu-august-overview-feca860-rollback-$stamp"
log="$root/logs/qianliu-zhisuan/deploy-august-overview-feca860-$stamp.log"
started=0
frozen=0
pointer_changed=0
success=0
...
GIT_SSH_COMMAND='ssh -o BatchMode=yes' GIT_TERMINAL_PROMPT=0 \
  git -C "$release" fetch --depth 32 origin "$candidate"
...
# Bind the entire reviewed path set; preserve inherited historical evidence verbatim.
expected_paths_sha256=096a953851ddfcdfc2e01e19381a12d74aef832c318ba883197461fb8ca608b0
changed_files="$(git -C "$release" -c core.quotepath=false -c diff.renames=false diff --name-only "$expected_source..$candidate")"
actual_paths_sha256="$(printf '%s\n' "$changed_files" | LC_ALL=C sort | shasum -a 256 | awk '{print $1}')"
test "$actual_paths_sha256" = "$expected_paths_sha256"
