#!/usr/bin/env bash
# Settings UX release on production Mac mini.
# Includes:
# 1. Admin role change auto-save decoupling with error rollback
# 2. Custom role setup guide navigation and tooltip enhancements
# Preserves existing database baseline (0073_credential_chat_probe or 0074_runtime_notification_recipients).
set -Eeuo pipefail
umask 077
...
if test "$mode" = --preflight; then
  ...
  echo "健康状态: 正常"
  echo "发布内容: 系统设置账号角色即时保存与岗位配置指引 (无数据库变更)"
  echo "=========================================="
  exit 0
fi

stamp="$(date '+%Y%m%d-%H%M%S')"
release="$root/releases/qianliu-settings-ux-$stamp"
backup_image="qianliu-settings-ux-rollback-$stamp"
...
exec > >(tee -a "$root/logs/qianliu-zhisuan/deploy-settings-ux-$stamp.log") 2>&1

echo "=========================================="
echo "开始部署: 仟流智算系统设置交互优化 (分支: $branch)"
echo "时间: $stamp"
echo "=========================================="
