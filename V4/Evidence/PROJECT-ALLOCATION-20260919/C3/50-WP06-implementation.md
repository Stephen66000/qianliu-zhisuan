...
2. **补偿扫描** `project-allocation-scan.ts`：worker 阶段一变更识别——不接入 Gateway/结算路径（B02 最严格解释）；按企业 `ledger_line.created_at` 水位+10 分钟回看，标记"晚于该月上次标记时间"的新行所在账期（防回看期重复推进代次），推脏→登记（SYSTEM actor）→执行到期批次；水位 per-enterprise upsert。
